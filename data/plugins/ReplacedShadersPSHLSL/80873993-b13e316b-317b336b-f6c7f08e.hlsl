  ...
  out float4 o0 : SV_TARGET0)
{
  ...
  bitmask.x = ((~(-1 << 1)) << 1) & 0xffffffff;  r0.x = (((uint)v8.x << 1) & bitmask.x) | ((uint)0 & ~bitmask.x);
  r0.x = (int)r0.x + -1;
  r0.x = (int)r0.x;
  r0.yz = float2(0.699999988,0.699999988) * cb0[23].xy;
  r1.xy = v6.xy / v6.ww;
  r1.xy = r1.xy * float2(0.5,-0.5) + float2(0.5,0.5);
  r0.x = v7.y * r0.x + v6.z;
  r1.z = saturate(r0.x / v6.w);
  r0.x = -1 + cb0[24].x;
  r0.w = 360 / r0.x;
  r2.z = 0;
    r2.w = 0;
  r3.x = 0;
  {
    ...
    for (int j = 0; j < 8; j++) {
      ...
      r3.z = r0.z * rotY + r1.y;
      r3.w = r1.z;
      r2.x = t1.SampleCmpLevelZero(s1_s, r3.yz, r3.w).x;
      r2.w = r2.x + r2.w;
    }
  }
  r0.x = cmp(0 < cb0[22].x);
  ...
  r1.x = cmp(r1.x < 0);
  r1.x = r1.x ? r1.y : 0;
  r1.y = saturate(dot(-v5.xyz, cb0[17].xyz));
  r1.z = saturate(dot(-v5.xyz, cb0[18].xyz));
  r1.w = saturate(dot(-v5.xyz, cb0[19].xyz));
  r2.xyz = cb0[8].xyz * r1.xxx;
  r3.xyz = cb0[9].xyz * r1.zzz;
  r1.xyz = r2.xyz * r1.yyy + r3.xyz;
  r1.xyz = cb0[10].xyz * r1.www + r1.xyz;
  r1.w = dot(-v5.xyz, cb0[16].xyz);
  r2.xyz = cb0[11].xyz * r1.www + cb0[12].xyz;
  r2.xyz = cb0[1].xyz * r2.xyz;
  r1.xyz = r1.xyz * cb0[0].xyz + r2.xyz;
  r2.xyz = r1.xyz * r0.xyz;
  r1.w = max(cb0[20].z, v2.x);
  r1.w = min(cb0[20].w, r1.w);
  r1.w = cb0[21].w * r1.w;
