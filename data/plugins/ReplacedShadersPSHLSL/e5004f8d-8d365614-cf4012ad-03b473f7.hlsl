// ---- Created with 3Dmigoto v1.2.45 on Sun Aug 30 19:17:57 2026
Texture2D<uint4> t6 : register(t6);

Texture2D<float4> t4 : register(t4);

TextureCube<float4> t3 : register(t3);

Texture2D<float4> t2 : register(t2);

Texture2D<float4> t1 : register(t1);

Texture2D<float4> t0 : register(t0);

SamplerComparisonState s4_s : register(s4);

SamplerState s3_s : register(s3);

SamplerState s2_s : register(s2);

SamplerState s1_s : register(s1);

SamplerState s0_s : register(s0);

cbuffer cb0 : register(b0)
{
  float4 cb0[36];
}




// 3Dmigoto declarations
#define cmp -
Texture1D<float4> IniParams : register(t120);
Texture2D<float4> StereoParams : register(t125);


void main( 
  float4 v0 : SV_POSITION0,
  float4 v1 : COLOR0,
  float v2 : COLOR2,
  float2 w2 : TEXCOORD0,
  float4 v3 : TEXCOORD1,
  float4 v4 : TEXCOORD2,
  float4 v5 : TEXCOORD4,
  float4 v6 : TEXCOORD5,
  float4 v7 : TEXCOORD6,
  float4 v8 : TEXCOORD7,
  float4 v9 : TEXCOORD8,
  float4 v10 : TEXCOORD9,
  uint v11 : SV_IsFrontFace0,
  out float4 o0 : SV_TARGET0)
{
  float4 r0,r1,r2,r3,r4,r5;
  float ignAngle, ign_sin, ign_cos, spiralR, spiralTheta, rotX, rotY;
  uint4 bitmask, uiDest;
  float4 fDest;

  bitmask.x = ((~(-1 << 1)) << 1) & 0xffffffff;  r0.x = (((uint)v11.x << 1) & bitmask.x) | ((uint)0 & ~bitmask.x);
  r0.x = (int)r0.x + -1;
  r0.x = (int)r0.x;
  r0.yz = float2(0.699999988,0.699999988) * cb0[24].xy;
  r1.xy = v9.xy / v9.ww;
  r1.xy = r1.xy * float2(0.5,-0.5) + float2(0.5,0.5);
  r0.x = v10.y * r0.x + v9.z;
  r1.z = saturate(r0.x / v9.w);
  r0.x = -1 + cb0[25].x;
  r0.w = 360 / r0.x;
  r2.z = 0;
    r2.w = 0;
  r3.x = 0;
  {
    r2.w = 0;
    ignAngle = frac(dot(v0.xy, float2(0.06711056, 0.00583715)));
    ignAngle = frac(52.9829189 * ignAngle) * 2 * 3.14159265359;
    sincos(ignAngle, ign_sin, ign_cos);
    [unroll]
    for (int j = 0; j < 8; j++) {
      spiralR = sqrt((j + 0.5) / 8);
      spiralTheta = j * 2.39996323;
      sincos(spiralTheta, r4.x, r5.x);
      r2.x = r5.x * spiralR;
      r2.y = r4.x * spiralR;
      rotX = ign_cos * r2.x - ign_sin * r2.y;
      rotY = ign_sin * r2.x + ign_cos * r2.y;
      r3.y = r0.y * rotX + r1.x;
      r3.z = r0.z * rotY + r1.y;
      r3.w = r1.z;
      r2.x = t4.SampleCmpLevelZero(s4_s, r3.yz, r3.w).x;
      r2.w = r2.x + r2.w;
    }
  }
  r0.x = cmp(0 < cb0[23].x);
  if (r0.x != 0) {
    r0.yz = floor(v0.xy);
    r0.yz = (uint2)r0.yz;
    r1.x = (int)r0.y & 31;
    r1.y = (int)cb0[23].y;
    r1.zw = float2(0,0);
    r0.y = t6.Load(r1.xyz).x;
    r0.z = 1 << (int)r0.z;
    r0.y = (int)r0.z & (int)r0.y;
    r0.y = min(1, (uint)r0.y);
    r0.y = (int)-r0.y + 1;
  }
  r0.x = r0.x ? r0.y : 0;
  if (r0.x != 0) discard;
  r0.xyz = -cb0[22].xyz + v8.xyz;
  r0.w = dot(r0.xyz, r0.xyz);
  r0.w = rsqrt(r0.w);
  r0.xyz = r0.xyz * r0.www;
  r1.xyz = t0.Sample(s0_s, w2.xy).xyz;
  r2.xy = t1.Sample(s1_s, w2.xy).yw;
  r2.xy = float2(-0.5,-0.5) + r2.yx;
  r2.xy = r2.xy + r2.xy;
  r0.w = -r2.x * r2.x + 1;
  r0.w = -r2.y * r2.y + r0.w;
  r0.w = max(0, r0.w);
  r1.w = cmp(0 < r0.w);
  r2.z = rsqrt(r0.w);
  r0.w = r2.z * r0.w;
  r0.w = r1.w ? r0.w : 0;
  r3.xyz = v6.xyz * r2.yyy;
  r2.xyz = v5.xyz * r2.xxx + r3.xyz;
  r2.xyz = v7.xyz * r0.www + r2.xyz;
  r0.w = dot(r2.xyz, r2.xyz);
  r0.w = rsqrt(r0.w);
  r2.xyz = r2.xyz * r0.www;
  r3.xy = v4.xy * cb0[5].xx + v3.xy;
  r3.xy = t2.Sample(s2_s, r3.xy).xy;
  r3.xy = cb0[4].zz * r3.xy;
  r2.xy = r2.xy * cb0[4].yy + r3.xy;
  r0.w = dot(cb0[17].xyz, r2.xyz);
  r1.w = 1 / cb0[25].x;
  r3.x = r2.w * r1.w;
  r1.w = -r2.w * r1.w + 1;
  r2.w = saturate(v10.x);
  r1.w = r1.w * r2.w + r3.x;
  r0.w = cmp(r0.w < 0);
  r0.w = r0.w ? r1.w : 0;
  r1.w = saturate(dot(-r2.xyz, cb0[17].xyz));
  r2.w = saturate(dot(-r2.xyz, cb0[18].xyz));
  r3.x = saturate(dot(-r2.xyz, cb0[19].xyz));
  r3.yzw = cb0[8].xyz * r0.www;
  r4.xyz = cb0[9].xyz * r2.www;
  r3.yzw = r3.yzw * r1.www + r4.xyz;
  r3.xyz = cb0[10].xyz * r3.xxx + r3.yzw;
  r0.w = dot(-r2.xyz, cb0[16].xyz);
  r4.xyz = cb0[11].xyz * r0.www + cb0[12].xyz;
  r4.xyz = cb0[1].xyz * r4.xyz;
  r3.xyz = r3.xyz * cb0[0].xyz + r4.xyz;
  r4.xyz = cb0[26].xyz + -v8.xyz;
  r4.xyz = cb0[26].www * r4.xyz;
  r0.w = dot(r4.xyz, r4.xyz);
  r0.w = sqrt(r0.w);
  r1.w = dot(r4.xyz, r2.xyz);
  r1.w = saturate(r1.w / r0.w);
  r0.w = 1 + -r0.w;
  r0.w = max(0, r0.w);
  r0.w = r1.w * r0.w;
  r4.xyz = cb0[28].xyz + -v8.xyz;
  r4.xyz = cb0[28].www * r4.xyz;
  r1.w = dot(r4.xyz, r4.xyz);
  r1.w = sqrt(r1.w);
  r2.w = dot(r4.xyz, r2.xyz);
  r2.w = saturate(r2.w / r1.w);
  r1.w = 1 + -r1.w;
  r1.w = max(0, r1.w);
  r1.w = r2.w * r1.w;
  r4.xyz = cb0[29].xyz * r1.www;
  r4.xyz = cb0[27].xyz * r0.www + r4.xyz;
  r5.xyz = cb0[30].xyz + -v8.xyz;
  r5.xyz = cb0[30].www * r5.xyz;
  r0.w = dot(r5.xyz, r5.xyz);
  r0.w = sqrt(r0.w);
  r1.w = dot(r5.xyz, r2.xyz);
  r1.w = saturate(r1.w / r0.w);
  r2.w = 1 + -r0.w;
  r2.w = max(0, r2.w);
  r1.w = r2.w * r1.w;
  r2.w = dot(cb0[31].xyz, -r5.xyz);
  r0.w = r2.w / r0.w;
  r0.w = -cb0[32].w + r0.w;
  r0.w = saturate(cb0[31].w * r0.w);
  r5.xyz = cb0[32].xyz * r1.www;
  r4.xyz = r5.xyz * r0.www + r4.xyz;
  r5.xyz = cb0[33].xyz + -v8.xyz;
  r5.xyz = cb0[33].www * r5.xyz;
  r0.w = dot(r5.xyz, r5.xyz);
  r0.w = sqrt(r0.w);
  r1.w = dot(r5.xyz, r2.xyz);
  r1.w = saturate(r1.w / r0.w);
  r2.w = 1 + -r0.w;
  r2.w = max(0, r2.w);
  r1.w = r2.w * r1.w;
  r2.w = dot(cb0[31].xyz, -r5.xyz);
  r0.w = r2.w / r0.w;
  r0.w = -cb0[35].w + r0.w;
  r0.w = saturate(cb0[34].w * r0.w);
  r5.xyz = cb0[35].xyz * r1.www;
  r4.xyz = r5.xyz * r0.www + r4.xyz;
  r3.xyz = r4.xyz + r3.xyz;
  r4.xyz = r2.xyz + r2.xyz;
  r0.w = dot(-r2.xyz, r0.xyz);
  r0.xyz = r4.xyz * r0.www + r0.xyz;
  r0.xyz = t3.Sample(s3_s, r0.xyz).xyz;
  r0.xyz = r0.xyz + r0.xyz;
  r2.xyz = cb0[3].xyz * r0.xyz;
  r4.xyz = float3(0.00999999978,0.00999999978,0.00999999978) + r3.xyz;
  r0.w = dot(r4.xyz, r4.xyz);
  r0.w = rsqrt(r0.w);
  r4.xyz = saturate(r4.xyz * r0.www);
  r2.xyz = r4.xyz * r2.xyz;
  r4.xyz = float3(1.73204994,1.73204994,1.73204994) * r2.xyz;
  r0.xyz = r3.xyz * r0.xyz;
  r0.xyz = r0.xyz * r1.xyz;
  r0.x = dot(r0.xyz, r0.xyz);
  r0.x = sqrt(r0.x);
  r0.x = 0.57735002 * r0.x;
  r0.x = min(1, r0.x);
  r0.yzw = cb0[2].xyz * r3.xyz;
  r1.xyz = r2.xyz * float3(1.73204994,1.73204994,1.73204994) + -r0.yzw;
  r0.yzw = r0.xxx * r1.xyz + r0.yzw;
  r0.yzw = -r2.xyz * float3(1.73204994,1.73204994,1.73204994) + r0.yzw;
  r0.yzw = cb0[2].www * r0.yzw + r4.xyz;
  r0.x = max(cb0[3].w, r0.x);
  r0.x = cb0[4].w * r0.x;
  r1.x = max(cb0[20].z, v2.x);
  r1.x = min(cb0[20].w, r1.x);
  r1.x = cb0[21].w * r1.x;
  r1.yzw = cb0[21].xyz + -r0.yzw;
  o0.xyz = r1.xxx * r1.yzw + r0.yzw;
  o0.w = v1.w * r0.x;
  return;
}